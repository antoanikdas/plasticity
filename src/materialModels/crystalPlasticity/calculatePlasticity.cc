...
void crystalPlasticity<dim>::calculatePlasticity(unsigned int cellID,
  ...
  unsigned int StiffnessCalFlag)
  {
    ...
    //////////// Anik
    double W_e, W_p_inc, W_p_cum, W_total_inc, W_total_cum;
    //////////////////////////////////////////////////////////
    ...
    //////////////////////////////////////////////////////////////////////
    // Elastic Energy (current state) Anik
    FullMatrix<double> Fe_minus_I(dim, dim);
    FullMatrix<double> I(dim, dim);  // Temporary identity matrix as FullMatrix
    I = IdentityMatrix(dim);         // Convert IdentityMatrix to FullMatrix
    Fe_minus_I = FE_tau;             // Set Fe_minus_I to FE_tau
    Fe_minus_I.add(-1.0, I);         // Subtract I: Fe_minus_I = FE_tau - I
    W_e = 0.0;
    for (unsigned int i = 0; i < dim; i++) {
      for (unsigned int j = 0; j < dim; j++) {
        W_e += 0.5 * P_tau[i][j] * Fe_minus_I[i][j];
      }
    }
  
    stateVar_iter[cellID][quadPtID][dim * dim + 4 * n_Tslip_systems + 1] = W_e;

    // Plastic Energy (cumulative) Anik
    // FullMatrix<double> dFp(dim, dim);
    // dFp.add(1.0, FP_tau, -1.0, FP_t); // FP_tau - FP_t
    // FullMatrix<double> Fe_dFp(dim, dim);
    // FE_tau.mmult(Fe_dFp, dFp);
    // W_p_inc = 0.0;
    // for (unsigned int i = 0; i < dim; i++) {
    //   for (unsigned int j = 0; j < dim; j++) {
    //     W_p_inc += P_tau[i][j] * Fe_dFp[i][j];
    //   }
    // }
    W_p_inc = 0;
    for (unsigned int alpha = 0; alpha < n_Tslip_systems; alpha++) {
      W_p_inc += resolved_shear_tau(alpha) * delgam_tau(alpha);
    }
    W_p_cum = stateVar_conv[cellID][quadPtID][dim * dim + 4 * n_Tslip_systems + 2] + W_p_inc;
    stateVar_iter[cellID][quadPtID][dim * dim + 4 * n_Tslip_systems + 2] = W_p_cum;

    // Total Energy (cumulative)
    FullMatrix<double> dF(dim, dim);
    FE_t.mmult(F_t, FP_t); // Recompute F_t since it’s not stored
    dF.add(1.0, F_tau, -1.0, F_t); // F_tau - F_t
    W_total_inc = 0.0;
    for (unsigned int i = 0; i < dim; i++) {
      for (unsigned int j = 0; j < dim; j++) {
        W_total_inc += P_tau[i][j] * dF[i][j];
      }
    }

    W_total_cum = stateVar_conv[cellID][quadPtID][dim * dim + 4 * n_Tslip_systems + 3] + W_total_inc;
    stateVar_iter[cellID][quadPtID][dim * dim + 4 * n_Tslip_systems + 3] = W_total_cum;


    // Fatigue Indicator Parameter (FIP_p = int sqrt(2/3 Lp : Lp) dt)
    FullMatrix<double> Lp(dim, dim);
    Lp = 0.0;
    for (unsigned int alpha = 0; alpha < n_Tslip_systems; alpha++) {
      FullMatrix<double> S_alpha(dim, dim);
      for (unsigned int i = 0; i < dim; i++) {
        for (unsigned int j = 0; j < dim; j++) {
          S_alpha[i][j] = SCHMID_TENSOR1[dim * alpha + i][j];
        }
      }
      double gamma_dot_alpha = delgam_tau(alpha) / this->userInputs.delT; // Approximate gamma_dot
      Lp.add(gamma_dot_alpha, S_alpha); // Lp += gamma_dot_alpha * S_alpha
    }
    double Lp_norm = 0.0;
    for (unsigned int i = 0; i < dim; i++) {
      for (unsigned int j = 0; j < dim; j++) {
        Lp_norm += Lp[i][j] * Lp[i][j]; // Compute Lp : Lp as sum of squares
      }
    }
    Lp_norm = sqrt((2.0 / 3.0) * Lp_norm); // sqrt(2/3 Lp : Lp)
    double FIP_p_inc = Lp_norm * this->userInputs.delT; // Increment: sqrt(2/3 Lp : Lp) * dt
    double FIP_p = stateVar_conv[cellID][quadPtID][dim * dim + 4 * n_Tslip_systems + 4] + FIP_p_inc;
    stateVar_iter[cellID][quadPtID][dim * dim + 4 * n_Tslip_systems + 4] = FIP_p_inc;


    // Fatigue Indicator Parameter for Fatigue Strength (Delta FIP_FS)
    double delta_FIP_FS = 0.0;
    for (unsigned int alpha = 0; alpha < n_Tslip_systems; alpha++) {
      double delta_gamma_p_alpha = delgam_tau(alpha); // Approximate cyclic range as incremental strain
      double sigma_n_alpha = normal_stress_tau(alpha);
      double tau_crit = 235.0; // Critical resolved shear stress
      double kappa = 0.55; // New material parameter
      double FIP_alpha = (delta_gamma_p_alpha / 2.0) * (1.0 + kappa * sigma_n_alpha / tau_crit);
      delta_FIP_FS = std::max(delta_FIP_FS, FIP_alpha);
    }
    stateVar_iter[cellID][quadPtID][dim * dim + 4 * n_Tslip_systems + 5] = delta_FIP_FS;
    
    // Fatigue Indicator Parameter for Fatigue Strength Transition (FIP_FS_trans)
    double delta_gamma_p_max = 0.0;
    double sigma_n_max = 0.0;
    for (unsigned int alpha = 0; alpha < n_Tslip_systems; alpha++) {
      if (delgam_tau(alpha) > delta_gamma_p_max) {
        delta_gamma_p_max = delgam_tau(alpha); // Maximum incremental shear strain
      }
      if (normal_stress_tau(alpha) > sigma_n_max) {
        sigma_n_max = normal_stress_tau(alpha); // Maximum normal stress
      }
    }
    double tau_c0 = 235.0; // Initial critical resolved shear stress
    double kappa = 0.55; // Weighting factor
    double FIP_FS_trans = delta_gamma_p_max * (1.0 + kappa * sigma_n_max / tau_c0);
    stateVar_iter[cellID][quadPtID][dim * dim + 4 * n_Tslip_systems + 6] = FIP_FS_trans;
